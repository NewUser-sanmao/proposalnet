$().ready(function() {
	...
	proposalUtil.setSelect2PleaseChoose('/proposal/lbyblsx/selectAllSelect',null,'flid',$("#flidVal").val());
...
//	$("#tanr").val(content_sn);
	...
	if($("#submitType").val() == 1){
		...
		$.ajax({
			cache : true,
			type : "POST",
			url : "/proposal/tasc/updateTascNoTastate",
			data : $('#signupForm').serialize(),// 你的formid
			async : false,
			error : function(request) {
				parent.layer.alert("Connection error");
